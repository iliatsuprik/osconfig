...
// Until checks immediately, then polls with a stoppable timer until success,
// a permanent error, or context cancellation.
func Until(ctx context.Context, interval time.Duration, description string, check Check) error {
	if interval <= 0 {
